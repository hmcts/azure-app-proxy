...
export async function setUserAssignmentRequired({token, objectId, assignmentRequired}: {
    token: string,
    objectId: string,
    assignmentRequired: boolean
}) {
    const result = await fetch(`https://graph.microsoft.com/v1.0/servicePrincipals/${objectId}`, {
        method: 'PATCH',
        headers: {
...
export async function findExistingServicePrincipal({token, displayName}: {
    ...
    const result = await fetch(`https://graph.microsoft.com/v1.0/servicePrincipals?$filter=displayName eq '${displayName}'&$top=1&$select=id`, {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${token}`,
        },
...
async function getAppRoleId(objectId: string, token: string) {
    const result = await fetch(`https://graph.microsoft.com/beta/servicePrincipals/${objectId}/appRoles`, {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${token}`,
        },
